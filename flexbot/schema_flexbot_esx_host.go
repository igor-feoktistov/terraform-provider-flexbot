...
// Schemas
func schemaEsxHost() map[string]*schema.Schema {
				...
				Schema: map[string]*schema.Schema{
					...
					"firmware": {
						...
						ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
							v := val.(string)
							if !(v == "bios" || v == "efi") {
								errs = append(errs, fmt.Errorf("value %q=%s must be either \"bios\" or \"efi\"", key, v))
							}
							return
						},
						Description: "The firmware type for ESX host service profile (bios or efi).",
					},
					"kernel_opt": {
						Type: schema.TypeString,
						Optional: true,
						Default: "ks=file:///ks.cfg",
						Description: "The ESXi installer kernel boot options (kernelopt) in boot.cfg",
					},
					"chassis_id": {
						Type:     schema.TypeString,
						Optional: true,
